...
public class ExamMainSolo {
    public static void main(String[] args) {
        ...
        int num = scan.nextInt(); // 사용자로부터 인원수를 입력받아 num 변수에 저장
        
        // Exam 객체 배열 생성
        Exam[] students = new Exam[num]; // 입력받은 인원수 크기의 Exam 객체 배열 생성
        
        // 각 학생의 Exam 객체 생성 및 초기화
        for (int i = 0; i < num; i++) {
            students[i] = new Exam(); // Exam 객체를 생성하여 배열의 각 요소에 할당
        }
        
        // 결과 출력
        System.out.println("\n이름\t1\t2\t3\t4\t5\t점수"); // 결과 표의 헤더를 출력
        for (Exam student : students) { // 학생 배열을 순회하며 각 학생의 정보를 출력
